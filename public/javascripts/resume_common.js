...
function activate_addSubPart(partBlock)
{
	/* The algo -
	//	create a clone of an existing subpart (the first one if there are many)
	// add an identifier to the cloned subpart
	// put all the :value fields within keyvalue pairs as null
	// display the input boxes for the values fields
	// add the button stripe for this
	// attach event handlers to the button stripe
	// attach the clone to the end of the subpart list */
	
	var firstSubPartBlock = partBlock.find(".resume_subpart").eq(0);
	var cloneSubPartBlock = firstSubPartBlock.clone(false)			// don't copy eventhandlers 
	cloneSubPartBlock.addClass("justAdded");						// add an identifier to the cloned subpart
		
	var allValueBlocks = cloneSubPartBlock.find(".value")
	allValueBlocks.each(function(){
		var displayElement = $(this).find(".display");
		var editElement = $(this).find(".edit");

		editElement.val("");										// put all the :value fields within 
																	// keyvalue pairs as null	
		displayElement.hide();				
		editElement.show();											// display the input boxes for the values fields
	});

	var button_stripe = $(".button_stripe").filter(".hidden").clone();
	button_stripe.removeClass("hidden");							// show and add the button_stripe
	button_stripe.addClass("button_stripe_active");
	cloneSubPartBlock.append(button_stripe);

	// add event handlers (for update and cancel) to the button stripe
	button_stripe.find(".update").click(function(e){ update_added_subPart(e) });			
	button_stripe.find(".clear").click(function(e){ deactivate_addSubPart(partBlock) });
	
	partBlock.find(".part_data").append(cloneSubPartBlock);			// attach the clone to the end of the subpart list

}
function update_added_subPart(e)
{
	// send an ajax post request to add the  subpart

	var partBlock = $(e.target).parents(".resume_part").eq(0);
	var added_subPartBlock = partBlock.find(".justAdded").eq(0);
	
	var postUrl = "/create/subpart"									// for creating a new subpart
	var postData = new Object();
	var authToken = $("*[name='authenticity_token']").eq(0).val();

	postData["ajax"] = "true";
	postData["authenticity_token"] = authToken;
	added_subPartBlock.find(".edit").each(function(){
		postData[$(this).attr('name')] = $(this).val();
	});

	var button_stripe = added_subPartBlock.find(".button_stripe");
	var ajaxMsgObj = button_stripe.find(".ajax_processing_msg");
	ajaxMsgObj.show();
	...
	$.post(
		...
		postData,
		function(response){
			add_subPart_callback(e,response);
		});
}
function add_subPart_callback(e, response)
{
	var responseObj = JSON.parse(response);
	...
	var id_list = responseObj["id_list"];
	var part_id = id_list[0]
	var subpart_id = id_list[1];
		
	// update the ids and names	: for the subpart title
	var subpart_block = $(e.target).parents(".resume_subpart").eq(0);
	var subpart_title_block = subpart_block.find(".subpart_title").eq(0);
	var title_element = subpart_title_block.find(".edit").eq(0);
	title_element.attr("id","resume_parts_"+part_id+"_subparts_"+subpart_id+"_title");
	title_element.attr("name","resume[parts]["+part_id+"][subparts]["+subpart_id+"][title]");
	
	// update the ids and names	: for the each of the kvps
	count = 2;
	var kvp_blocks = subpart_block.find(".keyvaluepair");
	kvp_blocks.each(function(){
		kvp_id = id_list[count];	count++;

		var key_element = $(this).find(".key > .edit");
		key_element.attr("id","resume_parts_"+part_id+"_subparts_"+subpart_id+"_keyvaluepairs_"+kvp_id+"_key");
		key_element.attr("name","resume[parts]["+part_id+"][subparts]["+subpart_id+"][keyvaluepairs]["+kvp_id+"][key]");

		var value_element = $(this).find(".value > .edit");
		value_element.attr("id","resume_parts_"+part_id+"_subparts_"+subpart_id+"_keyvaluepairs_"+kvp_id+"_value");
		value_element.attr("name","resume[parts]["+part_id+"][subparts]["+subpart_id+"][keyvaluepairs]["+kvp_id+"][value]");
		
	});
	
	// For each edit element, update the corresponding display element with the value
	subpart_block.find(".edit").each(function(){
		var editElement = $(this);
		var displayElement = $(this).siblings(".display").eq(0);
		displayElement.html(editElement.val());
	});
	
	// Hide each edit element, and show the display elements
	subpart_block.find(".edit").hide();
	subpart_block.find(".display").show();
	
	// Remove the button stripe
	subpart_block.find(".button_stripe").remove();

	// add eventhandlers for the subpart (while cloning, events were not cloned)
	eventHandlers_for_display_elements(subpart_block.find(".display"));
	
	// remove the justAdded classname from the subpart
	subpart_block.removeClass("justAdded");
	
	// show the plusbutton
	var partBlock = subpart_block.parents(".resume_part").eq(0);
	var plusButton = partBlock.find(".new_subpart_button");
	plusButton.show();	
}

function deactivate_addSubPart(partBlock)
{
	// find the just added subpart and remove it from the DOM
	var justAddedBlock = partBlock.find(".justAdded");
	justAddedBlock.remove();
